...
*)
  ...
  let of_project ?(meta=true) ~build_dir proj =
(*  TODO
    let r = As_resolver.create ~build_dir () in
    let name = As_project.name t in
    let parts = As_project.parts t in
    let libs = As_parts.(filter_map lib_ocaml parts) in
    let bins =
      List.filter As_parts.Bin.install
        As_parts.(filter_map bin parts)
    in
    ...
    if bins <> [] then (
      bprintf buf "bin: [\n";
      List.iter (fun b ->
          let gens = As_parts.files (`Bin b) in
          List.iter (fun (flags, files) ->
              List.iter (fun file ->
                  let file = As_parts.file (`Bin b) r file in
                  bprintf buf "  \"%s%s\" {\"%s\"}\n"
                    (opt flags) file (As_parts.name (`Bin b))
                ) files;
            ) gens;
        ) bins;
      bprintf buf "]\n";
...
*)
    let name = Assemblage.Project.name proj in
    let contents = "" in
    { name; contents }
