...
let mk_flags r phase t =
  ...
  let var = As_component.id t ^ "." ^ suffix in
  let global = match As_component.parent t with
  | None   -> [sprintf "$(%s)" suffix]
  | Some p -> [sprintf "$(%s.%s)" (As_component.id p) suffix]
  in
  let flags = global @ fn (As_component.flags ~all:false t r) in
  As_makefile.Var.(var =?= `Strings flags)

(* Replace all flags value with a flag variable in which the value is
   stored. *)
let meta_flags t =
  let phases = As_component.phases t in
...
let mk_rule resolver t rule =
  let targets = As_component.Rule.files t resolver rule.As_action.targets in
  ...
  in
  let prereqs = As_component.Rule.files t resolver prereqs in
  let order_only_prereqs = As_component.Rule.files t resolver oo_prereqs in
...
module Unit: S with type t = As_component.comp_unit = struct

  type t = As_component.comp_unit

  let variable r t =
    let c = `Unit t in
    let name = match As_component.Unit.source_dir t with
    | None   -> As_component.name (`Unit t)
    | Some d -> d / As_component.name (`Unit t) in
    let phases = As_component.phases c in
    match As_component.Unit.kind t with
    | `OCaml ->
  ...
  let rule r t =
    let c = `Unit t in
    List.map (mk_rule r c) (As_component.rules c)

  let rules r ts = conmap (rule r) ts
end

module Other: S with type t = As_component.other = struct
  type t = As_component.other
  let variables _ _ = []
  let rules _ _ = []
end
...
module Lib: S with type t = As_component.lib = struct

  type t = As_component.lib

  let variable r t =
    As_makefile.Var.stanza
      ~doc:[sprintf "Library: %s" (As_component.name (`Lib t))]
      (let c = `Lib t in
       let cma = As_component.file c r `Cma in
       let a = As_component.file c r `A in
       let cmxa = As_component.file c r `Cmxa in
       let cmxs = As_component.file c r `Cmxs in
       let byte = [ byte_f, `Strings [cma] ] in
       let native = [ native_f, `Strings [a; cmxa] ] in
       let native_dynlink = [ native_dynlink_f, `Strings [cmxs] ] in
       let id = As_component.id c in
       let phases = As_component.phases c in
       let open As_makefile.Var in
       (id =:= `Strings [])
       :: (id =+= case (As_component.available c) byte)
       :: (id =+= case (As_component.available c) native)
       :: (id =+= case (As_component.available c) native_dynlink)
       :: (List.map (fun phase -> mk_flags r phase c) phases))

  let variables r ts =
    let targets = List.map (fun t -> As_component.id (`Lib t)) ts in
...
module Bin: S with type t = As_component.bin = struct

  type t = As_component.bin

  let variable r t =
    let c = `Bin t in
    As_makefile.Var.stanza
      ~doc:[sprintf "Binary: %s" (As_component.name c)]
      (let byte = As_component.file c r `Byte in
       let native = As_component.file c r `Native in
       let js = As_component.file c r `Js in
       ...
       :: (id =+= case (As_component.available c) byte)
       :: (id =+= case (As_component.available c) native)
       :: (id =+= case (As_component.available c) js)
       :: List.map (fun phase -> mk_flags r phase c) phases)

  let variables r ts =
    let targets = List.map (fun t -> As_component.id (`Bin t)) ts in
    let js_targets =
      List.filter As_component.Bin.js ts
      |> List.map (fun t -> As_component.id (`Bin t))
    in
    let open As_makefile.Var in
...
module Container: S with type t = As_component.container = struct
  type t = As_component.container
  let variable r t =
    let c = `Container t in
    let phases = As_component.phases c in
    As_makefile.Var.stanza
      ~doc:[sprintf "Directory: %s" (As_component.name c)]
      (List.map (fun phase -> mk_flags r phase c) phases)
...
module Test: S with type t = As_component.test = struct

  type t = As_component.test

  let variables _r ts =
    let targets = List.map (fun t -> As_component.Rule.phony_run (`Test t)) ts in
...
module Doc: S with type t = As_component.doc = struct

  type t = As_component.doc

  let variable r t =
    let c = `Doc t in
    As_makefile.Var.stanza
      ~doc:[sprintf "Documentation: %s" (As_component.name c)]
      [mk_flags r `Doc c]
  ...
  let variables r ts =
    let add_docs = List.map (fun d ->
        ...
        As_makefile.Var.("doc" =+= case As_features.true_
                                [As_component.available c,
                                 `String (As_component.id c)])) ts
    in
...
let variables r ts =
  Pkg.variables r As_component.(filter_map pkg ts) @
  Lib.variables r As_component.(filter_map lib ts) @
  Bin.variables r As_component.(filter_map bin ts) @
  Test.variables r As_component.(filter_map test ts) @
  Doc.variables r As_component.(filter_map doc ts) @
  Container.variables r As_component.(filter_map container ts) @
  Unit.variables r As_component.(filter_map unit ts) @
  Other.variables r As_component.(filter_map other ts)

let rules r ts =
  Lib.rules r As_component.(filter_map lib ts) @
  Bin.rules r As_component.(filter_map bin ts) @
  Test.rules r As_component.(filter_map test ts) @
  Doc.rules r As_component.(filter_map doc ts) @
  Container.rules r As_component.(filter_map container ts) @
  Unit.rules r As_component.(filter_map unit ts) @
...
let of_project ?(buildir="_build") ?(makefile="Makefile") ?(clean_files=[])
  ...
  let distclean =
    As_makefile.Rule.create ~ext:true ~targets:["distclean"]
      ~prereqs:["clean"] [
      sprintf "rm -f %s %s.install META %s" makefile  (As_project.name t)
        (String.concat " " clean_files)
    ] in
  let install =
       ...
       @ (List.fold_left (fun acc -> function
         | `Lib _ | `Bin _ as c ->
             sprintf "@echo ' - %s -- build the %s %s.'"
               (As_shell.color `Underline (As_component.id c))
               (match c with `Lib _ -> "library"
                           | `Bin _ -> "executable"
                           | _ -> "component")
               (As_component.name c) :: acc
         | _ -> acc
         ) [] components
          |> List.rev)
  ...
  let phony =
    ...
    (List.fold_left (fun acc c ->
         ...
         | `Test _ -> id :: As_component.Rule.phony_run c :: acc
         | _ -> acc
       ) [] components
     |> List.rev)
  in
  let opt_includes =
    let units = As_component.(filter_map unit) components in
    ([], ["Makefile.assemble"]) ::
    [["clean"; "help"; "distclean"],
     conmap (fun u ->
         let mk f =
           if not (As_component.Unit.has (f:>As_action.file) u) then []
           else [As_component.file (`Unit u) resolver (`Dep f)]
         in
         mk `Ml @ mk `Mli
       ) units]
  in
