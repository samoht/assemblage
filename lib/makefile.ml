...
module Unit: sig
  ...
  let process t lib conf =
    ...
    let p4flags = p4oflags t lib conf in
...
module Lib: sig
  ...
  let rules t conf =
    ...
    let file ext     = "$(DESTDIR)" / Lib.name t ^ ext in
    let cma =
    ...
    let cmxa =
      ...
      let cmi = List.map (fun u -> file_u u  ".cmi") (Lib.units t) in
      Rule.create "cmxa" [file ".cmxa"] (cmx @ cmi) [
        sprintf "$(OCAMLOPT) -a %s -o %s" (String.concat " " cmx) Rule.target
      ] in
...
module Top: sig
  ...
  let rules t conf =
    ...
    let cma =
      deps
      |> Dep.get_local_libs
      |> List.map (fun l -> "$(DESTDIR)" / Lib.name l ^ ".cma") in
...
module Bin: sig
  ...
  let variables t conf =
    ...
    Variable.(n := link)
    :: List.concat (List.map (fun u -> Unit.variables u (Some l) conf) (Bin.units t))
  ...
  let rules t conf =
    ...
    let libs =
      ...
      |> Dep.get_local_libs
      |> List.map (fun l ->
          if Conf.native conf then "$(DESTDIR)" / Lib.name l ^ ".cmxa"
          else "$(DESTDIR)" / Lib.name l ^ ".cma") in
    let file u ext =
    ...
    :: List.concat (List.map (fun u -> Unit.rules u (Some l) conf) (Bin.units t))
