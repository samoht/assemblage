...
let path = function (`File p | `Effect p) -> p
let raw_path p = As_path.to_string (path p)
let basename p = As_path.basename (path p)
let dirname p = As_path.(as_rel (dirname (path p)))

(* Predicates *)

let is_file = function `File _ -> true | _ -> false
let is_effect = function `Effect _ -> true | _ -> false

let has_ext ext = function (* TODO also act on `Effect ? *)
| `File p when As_path.has_ext ext p -> true
| _ -> false

let keep_ext ext = function (* TODO also act on `Effect ? *)
| `File p as product when As_path.has_ext ext p -> Some product
| _ -> None
